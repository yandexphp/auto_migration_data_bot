...
export class SceneThirteen {
    private static page: Page

    constructor(page: Page) {
        SceneThirteen.page = page
        SceneThirteen.init()
    }
    ...
    public static async init() {
        try {
            ...
            const ENUM_FORM_COMPONENTS = {
                ...
                POSITION: 'position',
                FULL_NAME_PREDECESSOR: 'full_name_predecessor',
                FULL_NAME_APPOINTMENTS: 'full_name_appointments',
                RESULTS: 'results',
                STATUS: 'status',
                DATE: 'date',
            }

            Logger.log('SceneThirteen initialization.')

            const procedureMigration = async (pageIndex = 1) => {
                Logger.log('F[procedureMigration] - Procedure migration by page', pageIndex, '- starting')

                const url = getUrlIssueOfProjectByQueryId('ceo_and_procurement', '6527', pageIndex)
                ...
                for (const selectedId of ids) {
                    ...
                    if (SceneThirteen.getSuccessLoadedList().includes(selectedId)) {
                                    ...
                                    try {
                                        ...
                                        switch (key) {
                                            case ENUM_FORM_COMPONENTS.ORGANIZATION:
                                                fieldCode = '29'
                                                break;
                                            case ENUM_FORM_COMPONENTS.NAME_STRING:
                                                fieldCode = '3648'
                                                break;
                                            case ENUM_FORM_COMPONENTS.POSITION:
                                                fieldCode = '3655'
                                                break;
                                            case ENUM_FORM_COMPONENTS.FULL_NAME_PREDECESSOR:
                                                fieldCode = '3656'
                                                break;
                                            case ENUM_FORM_COMPONENTS.FULL_NAME_APPOINTMENTS:
                                                fieldCode = '3657'
                                                break;
                                            case ENUM_FORM_COMPONENTS.RESULTS:
                                                fieldCode = '3658'
                                                break;
