...
export class SceneSixteen {
    private static page: Page

    constructor(page: Page) {
        SceneSixteen.page = page
        SceneSixteen.init()
    }
    ...
    public static async init() {
        try {
            ...
            Logger.log('SceneSixteen initialization.')

            const procedureMigration = async (pageIndex = 1) => {
                Logger.log('F[procedureMigration] - Procedure migration by page', pageIndex, '- starting')

                const url = `https://ekap.kazatomprom.kz/projects/main_lean/issues?c%5B%5D=cf_29&c%5B%5D=subject&c%5B%5D=author&c%5B%5D=cf_144&c%5B%5D=cf_3&c%5B%5D=cf_7&c%5B%5D=cf_187&c%5B%5D=cf_97&c%5B%5D=cf_55&c%5B%5D=cf_4&c%5B%5D=cf_193&c%5B%5D=status&f%5B%5D=status_id&f%5B%5D=tracker_id&f%5B%5D=&group_by=cf_29&op%5Bstatus_id%5D=%2A&op%5Btracker_id%5D=%3D&per_page=750&page=${pageIndex}&set_filter=1&sort=cf_29%3Adesc%2Csubject&t%5B%5D=cf_187&t%5B%5D=cf_193&t%5B%5D=&utf8=%E2%9C%93&v%5Btracker_id%5D%5B%5D=10`
                ...
                for (const selectedId of ids) {
                    ...
                    if (SceneSixteen.getSuccessLoadedList().includes(selectedId)) {
                            ...
                            await Promise.all(formProcessDetail.sections.map(async ({
                                ...
                                const inputsList = await Promise.all(inputs.map(async ({
                                    ...
                                    try {
                                        ...
                                        switch (key) {
                                            case ENUM_FORM_COMPONENTS.EFFECT:
                                                fieldCode = '3'
                                                break;
                                            case ENUM_FORM_COMPONENTS.OTHER_EFFECT:
                                                fieldCode = '7'
                                                break;
                                            case ENUM_FORM_COMPONENTS.TENGE1:
                                                fieldCode = '187'
                                                break;
                                            case ENUM_FORM_COMPONENTS.DATAENASF:
                                                fieldCode = '2634'
                                                break;
                                            case ENUM_FORM_COMPONENTS.FILES:
                                                value = uploadedAttachments.map(({ bp_id }) => bp_id) ?? []
                                                break;
                                            case ENUM_FORM_COMPONENTS.TEMA:
                                            case ENUM_FORM_COMPONENTS.DATA_START:
                                            case ENUM_FORM_COMPONENTS.DATABEG:
                                            case ENUM_FORM_COMPONENTS.OPISANIE:
                                                fieldCode = null

                                                switch (key) {
                                                    case ENUM_FORM_COMPONENTS.TEMA:
                                                        value = xmlDocument.issue.subject ?? ''
                                                        break;
                                                    case ENUM_FORM_COMPONENTS.DATA_START:
                                                        value = xmlDocument.issue.created_on ?? ''
                                                        break;
                                                    case ENUM_FORM_COMPONENTS.DATABEG:
                                                        value = xmlDocument.issue.closed_on ?? ''
                                                        break;
                                                    case ENUM_FORM_COMPONENTS.OPISANIE:
                                                        value = xmlDocument.issue.description ?? ''
                                                        break;
                                                    default:
                                                }
                                                break;
